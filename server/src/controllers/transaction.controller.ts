...
export const processTransactions = async (
	...
	try {
		...
		if (error instanceof Error) {
			...
			res.status(500).json({
				message: 'Failed to process transaction files.',
				error: error.message,
			});
		} else {
...
export const getRejectedTransactions = async (
	req: Request,
	res: Response,
): Promise<void> => {
	try {
		...
		console.log(
			'Error retrieving rejected transactions:',
			(error as Error).message,
		);
		res
			.status(500)
			.json({ message: 'Failed to retrieve rejected transactions.' });
	}
};

export const getSummaryByCardType = async (
	req: Request,
	res: Response,
): Promise<void> => {
	try {
		...
		console.log(
			'Error retrieving summary by card type:',
			(error as Error).message,
		);
		res
			.status(500)
			.json({ message: 'Failed to retrieve summary by card type.' });
	}
};
...
export const getSummaryByCard = async (
	req: Request,
	res: Response,
): Promise<void> => {
	try {
