...
export const useTransactionStore = create<AppState>((set, get) => ({
		...
		fetchInitialData: async (apiKey: string) => {
			...
			try {
				const [cardTypeSummary, daySummary, cardSummary, rejected] = await Promise.all([
				...
				if (cardSummary.length > 0 || daySummary.length > 0 || rejected.length > 0) {
					const totalProcessed = cardSummary.reduce((sum, item) => sum + item.transactionCount, 0);
					set({
							processResult: { processed: totalProcessed, rejected: rejected.length },
